...
def is_interesting(
    ...
    """
    Determines if a seed is 'interesting' based only on:
    - New input/output (path/response) combinations.
    - New/unseen response codes.
    - Optional: Unusual command sequence length.
    """
    ...
    # 3. (Optional) Mark long sequences as interesting
    if len(seed.data) > 5:
        return True
    ...
    try:
        while seed_queue:
            ...
            # for _ in range(1):  # Run mutations once for each seed
                ...
                mutated_seed = Seed(
                    priority=random.random(),  # Could be adjusted heuristically
                    energy=energy,
                    data=mutated_data,
